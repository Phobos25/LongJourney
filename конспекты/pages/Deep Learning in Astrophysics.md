...
- ## 2.0 Развитие глубокого обучения в астрофизике
  collapsed:: true
	...
	- Ключевая проблема в глубоком обучение это т.н. **inductive bias**, который есть в машинном обучении. Т.е. когда мы сознательно вносим некоторое смещение или, можно сказать, предвзятость в модель, чтобы получить определенное решение. Как бы подталкивая модель к нему. Это делается потому, что мы вносим ограничения, которые даются теоретическими моделями физики, которые необходимо учитывать при предсказании с помощью машинного обучения.
...
- ## 2.2 Выбор архитектуры для определенной гипотезы
  collapsed:: true
	...
	- ### 2.2.2 RNN
	  collapsed:: true
		- Рекурентные нейросети тоже используются в астрономии, но в данных, которые разбиты по времени (time series), хотя они постепенно начинают заменяться трансформерами
		- Архитектура модели позволяет рассматривать данные "в развитии", т.е. интервал данных рассматривается как развитие какого-то процесса. Мы можем рассматривать предыдущие данные как "память". В астрономии это взрыв сверхновой например. Или светимость звезды, которая меняется со временем (для этого способность модели "запоминать" предыдущие по времени данные отлично подходит).
	...
	- ### 2.4 Encoding Physical Symmetries
	  collapsed:: true
		- В физике есть симметричность, которую надо запрограммировать в модель. Конечно есть и асимметричность, например, чтобы различать галактики (с какой стороны рукав, в какую сторону повернут и т.п.). Нам важно с какой стороны рукав, как  он повернут и т.п. С другой стороны светимость галактики не зависит от поворота и положения на рисунке.
...
- ## 3.5 Reinforcement learning and Workflow optimization
  collapsed:: true
	- Обучение с подкреплением это один из способов машинного обучения, в ходе которого агент обучается, взаимодействуя с некоторой средой. В астрофизике и астрономии это очень важно с точки зрения оптимизации работы. Потому что время в течении которого телескопы могут вести наблюдения ограничены, необходимо это время автоматизировать и свести к минимуму человеческий фактор. Чтобы данный процесс мог вести наблюдения и по полчаса в сутки, для чего никто не будет привлекать исследователя.
	- Модель должна обучаться по датчикам или по прогнозу погоды: наличие дождя, луны, ветра, время суток и т.п. Будет очень сложно написать код, который сможет принимать во внимание все эти параметры и принимать правильные решения. К тому же будет сложно учесть все случаи. Здесь приходит на помощь машинное обучение - обучение с подкреплением. Модель должна сама принимать решения в зависимости от текущих условий. Обучать ее можно прямо во время сессий наблюдений. Чтобы модель можно было проверить прямо на месте, и исправить в случае чего. В астрономии уже есть телескопы, которые работают полностью автоматически. Результаты полученные на них уже говорят о перспективности таких методов.
	- Более того, можно настроить модель таким образом, чтобы можно было настраивать детектор (подбор напряжения, усиления и т.п.).
- ## 3.6 Large Language Model and Agentic Research
  collapsed:: true
	- Несмотря на все успехи моделей глубокого обучения, машинного обучения и использования нейросетей, все же не следует забывать, что большая часть астрономических открытий приходит из простых исследований, которые не могут быть оценены с помощью функции потерь. Когда астрономы сталкиваются с аномалиями в спектре или неожиданной корреляцией они начинают генерировать гипотезы и подходить творчески к решению проблемы, которая основана на многолетнем опыте и интуиции. Это все наводит мысли о применении больших языковых моделей (LLM) для автономного научного поиска.
	- Концепция "беспилотных лабораторий" которая активно исследуется в материаловедении (Szymanski et al. 2023) и химии (Boiko et al. 2023), предполагает широкие возможности для астрономии. Такие системы анализируют десятки тысяч опубликованных работ и ведут автономные эксперименты. В астрономии уже появились похожие модели, которые могут по эффективности соперничать с самыми передовыми моделями (de Haan et al. 2025). В то время как LLM как второй пилот поднимает вопросы об индивидуальности ученых (Ting 2025a) и природе научной практики и образования (Ting & O'Briain 2025), но ее технические возможности заставляют крепко задуматься.
	- Можно ведь использовать модели не только для получения информации, но использовать для более амбициозных целей: сделать ИИ автономным, чтобы он планировал, исполнял и адаптировал методы для исследования без всякого вмешательства со стороны людей. Превратить ИИ в генератор гипотез.
	- Однако, парадокс Моравека опускает нас на землю - в то время как ИИ преуспевает в высококогнитивных процессах, он испытывает трудности в сенсомоторных процессах, здравом смысле, и восприятии которые люди находят тривиальными. В то время как ИИ достигают значительных результатов - включая золотую медаль на международной олимпиаде по астрономии и астрофизике (Carrit Delgado Pinheiro et al. 2025) - использования ИИ в качестве полностью автономного агента, а не второго пилота требует более целостных возможностей. Системы с участием человека могут позволить себе такие ошибки, которые полностью автономные системы не могут ни в коем случае. Базовые способности, такие как чтение графиков оставляют желать лучшего (Wang et al. 2024). Если исключить чисто математическую логику, написание кода в автономном режиме остается очень ограниченным, и по последним данным даже топовые модели не способны решить большинство задач по программированию с которыми сталкивается реальная наука (Chen et al. 2024).
	- И самое главное, в астрономии основная проблема не генерация гипотез, а их проверка. Предложить новую модель темной материи или модифицировать космологические параметры легко, однако, их проверка требует значительных вычислений, статистических выводов и детальное понимание эксперимента. Все это фундаментально изменяет использование моделей в астрономии, они должны использоваться для строительства экосистемы - разработка инструментов и интерфейсов, которая позволяет LLM правильно взаимодействовать с астрономическими данными, симуляциями и анализировать существующие инструменты, а не пытаться заниматься полностью автономными исследованиями.
- ## 4.1 Где глубокое обучение может иметь наибольший эффект
  collapsed:: true
	- Для астрономии характерны четыре источника проблем:
		- а) Ограниченное количество данных. Ввод новых телескопов в последнее время  таких как Gaia (Gaia Collaboration et al. 2023), LSST (Ivezic et al. 2019), Euclid (Euclid COllaboration et al. 2025), и Roman Space Telescopes позволяет увеличить количество данных, но этот процесс не зависит от ИИ;
		- б) Фундаментальная теория. Самая большая лакуна в наших знаниях - это теория. То, что даже высокоточные измерения не могут разрешить противоречия в теории показывает, что даже более совершенные техники не могут решить проблему в фундаментальной физике. Модели основанные на больших языковых моделях могут решить эту проблему, но это все еще остается просто спекуляцией;
		- в) Получение информации с данных. Очень сложно получить какую-то полезную информацию с многомерных данных, которые не подчиняются нормальному распределению. SBI и детектирование аномалий может помочь с решением данной проблемы:
		- г) Систематические ошибки симуляций. Нелинейные астрофизические системы требуют численные симуляции, которые страдают от аппроксимаций. Мульти-масштабное моделирование и фундаментальные модели могут помочь с этим.
		- Deep Learning может помочь с (в) и (г). Вопрос заключается в том, что из этих двух является более приоритетным?