...
Для сигмоидной функции данные должны быть центрированы на 0, т.е. среднее значение всех данных должно быть 0. 

Причина кроется в том, что если нейрон будет всегда позитивным, то градиент весов будет либо полностью положительным, либо полностью отрицательным. В следствии чего схождение будет идти зигзагообразно, что нежелательно. 

Поэтому, резюмируя мы можем скажать следующее про сигмоиодную функцию:

* Сигмоидная функция приводит данные к области [0,1]
* Исторически очень популярная функция
Но у нее есть три проблемы:
* Перенасыщение --- перенасыщенные нейроны "уничтожают" градиент
* Функция не центрирована на 0
* экспонента невыгодна с точки зрения кмопьютерных мощностей. 

следующая функция tanh(x) --- во всех случаях лучше сигмодиной функции. Она принимает значения [-1,1], но остальные проблемы сигмоидной функции остаются --- перенасыщение и не оптимальное время расчетов. 

ReLu --- rectified Linear Unit. Функция выглядит как f(x) = max(0,x), т.е. все значения ниже 0 уничтожаются, а значения выше 0 растут линейно. 

* Не перенасыщается
* Очень эффективен с точки зрения времени расчетов
* Сходится намного быстрее сигмоидной или tanh функций (примерно в 6 раз быстрее)

Недостатки:
* Не ориентирован на 0
* уничтожает все градиенты для значения ниже 0, что неудобно

# Leaky ReLU
f(x) = max(0.01x,x)
работает чуть лучше, чем обычный ReLU, еще один вариант f(x) = max($$\alpha$$x, x)

Единственное различие от ReLU в том, что этот вариант не уничтожает градиент. 

# Резюме
* Используйте ReLU. Будьте внимательны со степенью обучения
* Попробуйте Leaky ReLU / Maxout / ELU
* Можете попробовать tanh, но не следует ожидать слишком многого
* Не используйте сигмоидную функцию

# Data Preprocessing

предобработка данных очень важна для обучения нейросетей. Есть два основных способа такой обработки: центровка на 0 (zero-centered data)
и нормировка (normalized data).
Для центрирования можно использовать следующую команду: X -= np.mean(X, axis=0)
Для нормировки можно использовать следующую команду: X /= np.std(X, axis=0)

Для изображений используется центрирование на 0. Вычитается среднее изображение из всего изображения. Пример, если у нас есть
imageNet, вычитаемое среднее изображение будет оранжевым пятном. 
Для CIFAR-10 с [32,32,3] изображением среднее, тоже будет [32,32,3] изображением, которое будет состоять из среднего цвета. 
Можно еще вычесть средний цвет канала. Т.к. любое изображение можно разделить на три составляющие RGB (red, green, blue). Мы 
вычитаем эти цвета с изображения.
Такая предобработка позволяет оптимизировать процесс обучения нейросетей. 

Еще одной проблемой является инициализация весов. В начале использовали 0, но это оказалось большой проблемой, т.к. модель получалась симметричной и не обучалась, т.е. все веса были равны друг другу и вероятность для любого ответа была одинаковой. 
Одним из самых оптимальных вариантов является т.н. "Xavier initialization", который предполагает использование случайных чисел, и учитывает входные данные. Т.е. если входные данные слишком большие, то старается веса подобрать маленькие, если слишком маленькие, то большие. 
...
для ReLU надо использовать немного измененную формулу: 
```
...
Этот метод позволяет избежать оверфита, и использовать все возможные параметры для оптимизации модели. 
К примеру, если мы работаем с моделью для распознаванию кошек, наша модель могла начать слишком сильно использовать только определенные параметры (у определенных параметров слишком большие веса):
```
has an ear        x
has a tail
is furry          x  
...
big eyes
```
Слишком большой вес у шерстки кошек, ведь у некоторых кошек шерсти может не быть, или шерксть может быть слишком короткой. То же касается ушек, есть кошки у которых ушки торчком, есть вислоухие кошки. 
Еще один пример, можно рассматривать dropout, как использование нескольких ансамблей моделей. 
